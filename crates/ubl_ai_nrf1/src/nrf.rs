...
pub fn cid_from_nrf_bytes(bytes: &[u8]) -> Cid {
...
mod tests {
    ...
    fn golden_vector_cid_stability() {
        // This is the CID from the race card. If this changes, something broke.
        ...
        assert_eq!(
            cid.to_string(),
            "bafkreigibhh2eucyeberwvkqx56braqzvokd2d45jrg24d5iqcsoumjmrq",
            "CID for {{hello:world,n:42}} must be stable across builds"
        );
    }
    ...
    fn cid_deterministic_for_same_bytes() {
        ...
        assert_eq!(c1, c2);
        assert!(c1.to_string().starts_with("bafkrei"));
    }
