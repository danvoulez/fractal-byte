...
lazy_static! {
    static ref SIGNING_KEY: SigningKey = {
        ...
        bytes.copy_from_slice(&hex::decode("11223344556677889900aabbccddeeff11223344556677889900aabbccddeeff").unwrap());
        SigningKey::from_bytes(&bytes)
    };
    pub static ref VERIFYING_KEY: VerifyingKey = SIGNING_KEY.verifying_key();
    pub static ref ISSUER_DID: String = format!("did:key:z{}", bs58::encode(VERIFYING_KEY.to_bytes()).into_string());
}
...
pub async fn issue_receipt(cid: &Cid, bytes_len: usize) -> Result<String> {
    ...
    let payload_b64 = base64::engine::general_purpose::URL_SAFE_NO_PAD.encode(serde_json::to_vec(&receipt)?);
...
pub async fn get_receipt(cid: &Cid) -> Option<String> {
    ubl_ledger::get_receipt(cid).await.and_then(|b| String::from_utf8(b).ok())
}
